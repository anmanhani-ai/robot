...
class RobotBrain:
    ...
    def calculate_y_from_bottom(self, target) -> Tuple[float, float]:
        """
        คำนวณระยะ Y จากขอบล่างของภาพถึงขอบล่างของวัตถุ
        
        Args:
            target: Detection object with y and height properties
            
        Returns:
            Tuple[float, float]: (ระยะ_cm, เวลา_วินาที at 2.17 cm/s)
        """
        # bottom edge of object in pixel
        target_bottom_y = target.y + (target.height // 2)
        # distance from bottom edge of image
        pixels_from_bottom = 480 - target_bottom_y
        # convert to cm using calibrated pixel_to_cm ratio
        distance_cm = pixels_from_bottom * self.config.pixel_to_cm_z
        # calculate time using calibrated arm speed
    ...
    def align_to_target(self, distance_from_center_px: int) -> bool:
        """
        จัดตำแหน่งรถให้ target อยู่ตรงกลาง (เดินหน้า/ถอยหลัง)
        ...
        # เคลื่อนที่ (เดินหน้า/ถอยหลัง แทนการเลี้ยว)
        if direction == "FW":
            cmd = "MOVE_FORWARD"
        else:
            cmd = "MOVE_BACKWARD"
            
        if not self.send_cmd(cmd, wait_for_done=False):
            return False
